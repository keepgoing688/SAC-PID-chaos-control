...
本项目验证了由 SAC 在线整定 PID 增益的混合控制思路，可改善固定参数 PID 面对强非线性和时变动态时的适应性，并为数据驱动的结构化控制研究提供可复现实验基础。

- **复杂非线性系统镇定**：通过将 SAC 的在线学习能力与 PID 的可解释执行结构结合，探索在复杂非线性动态下兼顾收敛速度、稳态精度与鲁棒性的控制策略。
- **工程振动与流体系统控制**：该方法可为机械结构异常振动抑制、湍流或其他强非线性流体系统调控等研究任务提供仿真方法参考。
- **电力与神经动力学研究**：该框架可作为电力系统振荡抑制、神经元集群异常放电调控等方向的算法验证起点。

本仓库展示的是 Lorenz 与 Lorenz96 数值仿真结果；面向实际工程应用时，仍需进一步考虑测量噪声、控制时延、模型偏差和安全约束。
...
与论文表 3-3 对应的统计数据见 [stats_table.csv](lorenz/tables/stats_table.csv)。该文件还保留峰值误差和训练平均奖励，便于复现实验结果。稳态误差、RMSE 和收敛时间反映镇定性能，平均恢复时间反映抗扰能力。

| 指标 | 固定参数 PID | 端到端 SAC | SAC-PID 混合 |
| --- | ---: | ---: | ---: |
| 稳态误差 $\|e\|$ | 0.4337 | 0.0376 | **0.0056** |
| RMSE | 1.8969 | 2.1970 | **1.3012** |
| 收敛时间 / s | 1.330 | 0.790 | **0.270** |
| 平均恢复时间 / s | 1.0225 | 0.4850 | **0.2000** |
